...
- **`Mzinga.Core`**: Implements the base rules, piece movement validation, board state logic and the core AI algorithms providing heuristic evaluations.
- **`Mzinga.Engine`**: Acts as the executable shell, maintaining engine state and processing input commands using the Universal Hive Protocol (UHP).
...
- **Architecture-as-Code (AaC)** using **ArchUnitNET** to ensure system boundaries and dependencies remain isolated and **trxlog2html** to generate HTML reports.

## **Unit testing and code coverage**

The **Mzinga** project utilizes the **[MSTest](https://learn.microsoft.com/en-us/dotnet/core/testing/unit-testing-mstest-intro)** framework for its existing unit tests. Alongside the test framework, **[Coverlet](https://github.com/coverlet-coverage/coverlet)** is integrated as a cross-platform code coverage library for .NET. 
...
- **`Target`**: Indicates which context of tests to evaluate. Results and reports are separately cached under `Results/{Target}` and `CoverageReport/{Target}` directories.
  - **`original`**: Evaluates only existing unit tests located in the [Mzinga/src](./Mzinga/src) directory.
  - **`new`**: Evaluates only the new tests located in the [Unit Tests](./Unit%20Tests) directory.
...
2. `Exception` classes: Exception classes such as `CommandException`, `GameOverException`, `NoBoardException`, `PerftInvalidDepthException`, and `UndoInvalidNumberOfMovesException` have 0% coverage. This clearly shows that error handling and invalid state edge cases are currently not being verified by the test suite.
...
A new unit test project named [Mzinga.Tests.New](./Unit%20Tests/Mzinga.Tests.New) was initialized within the [Unit Tests](./Unit%20Tests/) directory. This MSTest project will hold new test cases focused on covering lines and branches the original tests do not cover. Tests were organized by classes:
...
-   **`EngineConfig_GetOptionsClone`**: Validates deep cloning for game settings.
...
-   **`Engine_ParseCommand_PlayWithoutArgs` / `Engine_ParseCommand_UndoWithoutArgs`**: Checks responses against playing and undoing actions with missing target parameters.
...
-   **`Engine_ParseCommand_Options` / `Engine_ParseCommand_OptionsGet` / `Engine_ParseCommand_OptionsSet`**: An aggregate check around `options` command variations. Assigns valid string configurations like variables, depths and flag bounds to confirm states propagate directly to the main `Engine` config structure. Incorporates multithreading behavior checks, bounds checks and exception logic flow ensuring invalid inputs are handled as an `ArgumentException`.
...
-   **`GameMetaData_MoveCommentary`**: Tests if move comments are correctly added.
...
Looking at the detailed results for the classes where tests were added ([Image 4](#img4)), we achieved substantial improvements:
...
- **`CommandException`**, **`NoBoardException`**, **`PerftInvalidDepthException`** and **`UndoInvalidNumberOfMovesException`** all increased from 0% to 100% line coverage.
...
The newly introduced unit tests successfully expanded original test suite and improved the project's overall code coverage. While we exceeded our target goal of 80% coverage, specific classes like `MoveSet` and `VersionUtils` can still benefit from further additions in the future.
...
- **No coverage**: Mutants in code that is not executed by any of the unit tests.
...
- **`EngineConfig.cs`** achieved a mutation score of **51.82%**.
- **`Engine.cs`** achieved a mutation score of **18.39%**.
...
We can see that the common mutants introduced include switching function calls with an empty command, negating expressions, changing string messages etc. as shown on images [8](#img8), [9](#img9) and [10](#img10).

<div id="img8" align="center">
  <img src="./Mutation Testing/Images/report4.png" alt="Survived mutant examples">
  <p><em>Image 8: Survived mutant examples</em></p>
</div>

<div id="img9" align="center">
  <img src="./Mutation Testing/Images/report5.png" alt="Survived mutant examples">
  <p><em>Image 9: Survived mutant examples</em></p>
</div>

<div id="img10" align="center">
  <img src="./Mutation Testing/Images/report6.png" alt="Survived mutant examples">
  <p><em>Image 10: Survived mutant examples</em></p>
</div>
...
The results of the mutation testing underscore the danger of relying solely on code coverage metrics. Despite reaching over 80% coverage, a massive portion of inserted defects slipped through the tests, meaning future effort must prioritize the quality and strictness of the test assertions.

## **Code formatting**

The original Mzinga codebase contains an [.editorconfig](./Mzinga/src/.editorconfig) file. This existing configuration only disables a few specific C# features, such as implicit object creation, range operators and switch expressions, but it completely lacks standard formatting rules to ensure code consistency.
...
- **`TargetDir`** (required): Specifies the name of the subdirectory inside the [Code Formatting/Results](./Code%20Formatting/Results) directory where the output report will be saved.
...
We can see that tool reports it can't fix `IDE 1006` warnings, which represent name rule violations ([Image 15](#img15)). The reason for this is that renaming symbols is a complex refactoring operation and automatic renaming could potentially break the codebase if those symbols are used in reflection, serialization or exposed via public APIs, so the tool refuses to fix them automatically and requires manual fixing. We won't be manually fixing naming violations in this analysis.
...
Generated report in [Code Formatting/Results/FormatApply](./Code%20Formatting/Results/FormatApply/) shows which erros and warnings were fixed. As we can see, everything but the name rule violations was fixed. To verify that, we can run the script in check mode again.
...
By enforcing standard code formatting, we improved the general consistency and readability of the codebase. Although we bypassed automated renaming of some internal and public symbols to avoid potential breaks, the code now follows a clear set of formatting rules.
...
- **`TargetDir`** (required): Specifies the name of the subdirectory inside the [Static Code Analysis/Results](./Static%20Code%20Analysis/Results) directory where the output report will be saved.
...
After initial check, we will run the tool in `apply` mode to automatically resolve the warnings.

```powershell
& ".\Static Code Analysis\run_roslynator.ps1" apply ApplyFixes -Visualize
```
...
Resolving `RCS1194` (Implement exception constructors) cannot be done automatically because generating those constructors affects the public API of a class. Standard [.NET guidelines](https://learn.microsoft.com/en-us/dotnet/standard/exceptions/how-to-create-user-defined-exceptions) dictate that custom exceptions should provide at least three default constructors - an empty constructor, one that takes a string message and one that takes both a string message and an inner exception.
...
Once the appropriate manual fixes for the empty catch blocks and exception constructors are chosen and applied to the source files, executing the `check` mode of the script once more will confirm that all warnings have been successfully resolved.

In conclusion, we discovered minimal issues, emphasizing that the source code was already robust and free of any severe bugs. Fixing the few minor anomalies presented ensures that code aligns with standard .NET best practices.
...
1. Installs `dotnet-trace`, if it doesn't already exist.
...
4. Starts a new game and instructs the engine to play 4 turns without profiling, allowing maximum of 60 seconds of search time per turn. This populates the board and game reaches a more complex state.
...
We will run the execution script with results visualization:
...
3. **`CalculateValidPlacements`**: Calculating valid placements for playing new pieces requires evaluating board adjacencies to make sure no opposing piece touches the newly placed piece.
...
To resolve this inefficiency, the `Board` class should implement incremental move generation. Instead of recalculating all valid moves from scratch on every evaluated turn, the engine should store and incrementally update a `ValidMoves` cache. 
...
Applying performance profiling successfully led us to the core bottlenecks nested deep within the internal search tree. By identifying the inefficient and repetitive execution of `GetValidMoves` inside `GetBoardMetrics`, we clearly pinpointed the need for incremental move generation as a way to scale up the Mzinga AI engine's performance.
...
- **`-Visualize`**: Parses the generated `.trx` result into an HTML document using the `trxlog2html` tool and automatically opens it in the default web browser.
...
2. **Engine Layer Isolation Rule**: `Mzinga.Engine` namespace classes hold execution logic but they should not depend directly on the GUI from `Mzinga.Viewer`.
3. **Testing Code Isolation Rule**: Production source code modules (`Mzinga.Core` and `Mzinga.Engine`) must not depend on any test utilities or mocks found in testing namespace (`Mzinga.Test`). This guarantees test dependencies naturally fall away when releasing to production.
